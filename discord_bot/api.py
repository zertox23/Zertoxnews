...
class Api:
    ...
    async def test_web(self,data:dict) -> json:
        payload = {"data":data}
        async with aiohttp.ClientSession() as session:
            async with session.post(self.api_url+"/api/test_scrape",json=payload,) as r:
    ...
    async def add_new_source(self,data:dict) -> json:
        payload = {"data":data}
        async with aiohttp.ClientSession() as session:
            async with session.post(self.api_url+"/api/add_source/",json=payload) as r:
                jso = await r.json()
                return jso