...
class Commands(commands.Cog):
    ...
    @app_commands.describe(cat="the category you want the bot to send news in")
    async def add_all_sources(
        self, interaction: discord.Interaction, cat: discord.CategoryChannel
    ):
        try:
            
            await interaction.response.defer()
            guild:discord.Guild = self.bot.get_guild(interaction.guild_id)
            async def add_channel(source):
                channel = await guild.create_text_channel(name=source,news=True,nsfw=True,category=cat)
                session.add(DbStruct.channels(source=source,channel_id=channel.id))
                session.commit()
            
            channels = session.query(DbStruct.channels).all()
            sources = session.query(DbStruct.sources).all()
            
            for source in sources:
                source = source.source
                if source not in [channel.source for channel in channels]:
                    await add_channel(source=source)
        except Exception as e:
            logger.error("Error in adding source" + e)                    
            ...
            data = {
                link: {
                    ...
                    "article_obj": {
                        ...
                        "attrs": {article_obj_attr: article_obj_attr_value or ""},
                    },
                    "href": {url_attr: url_attr_value or ""},
                }
            }
            ...
            if r:
                try:
                    ...
                    message = await interaction.followup.send(
                        "you have 30 seconds to decide if this response is correct or no",
                        file=file,
                    )
                    ...
                    try:
                        ...
                        if str(reaction.emoji) == "✅":
                            await interaction.followup.send(
                                f"adding the scrape configuration"
                            )
                            r = await api.add_new_source(data=data)
                            json_data = json.dumps(r, indent=4)
                            file_like_object = io.BytesIO(json_data.encode("utf-8"))
                            file = discord.File(
                                file_like_object, filename="response.json"
                            )
                            await interaction.followup.send(
                                "server response", file=file
                            )
                        elif str(reaction.emoji) == "❌":
                            pass
                    except asyncio.TimeoutError:
                        await interaction.followup.send(
                            "No reaction received within the timeout period. Skipping."
                        )
                except Exception as e:
    ...
    async def check_for_sources(self, interaction: discord.Interaction):
        try:
            ...
            if sources:
                try:
                    db_sources = [
                        x.source for x in session.query(DbStruct.sources).all()
                    ]
                    for source in sources:
                        try:
