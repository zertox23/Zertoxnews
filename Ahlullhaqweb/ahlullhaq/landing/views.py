...
def index(request, page: int = 1, source: str = None):
    if request.user.is_authenticated:
        ...
        if not source:
            ...
            articles = (
                Article.objects.filter(Article.author == source)
                .all()
...
def article(requst, id: int):
    if requst.user.is_authenticated:
        ...
        for med in media:
            ...
            if med.is_image == True and are_images_similar(
                med.file_data, art_media.file_data
            ):
                continue
            else:
...
def serve_media(request, media_id, main_image=None):
    ...
    response["Content-Disposition"] = (
        f'{content_disposition}; filename="media.{extension}"'
    )
