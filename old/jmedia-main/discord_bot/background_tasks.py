...
async def download_img_through_tor(url: str, folder: str, method: str = "GET"):
    ...
    if method.upper() == "GET" and ".mp4" not in url:
        try:
            r = requests.get(
                url, proxies={"http": tor_proxy}, headers={"User-agent": "Mozilla/5.0"}
            )
...
class BackgroundTasks(commands.Cog):
    ...
    async def get_latest_news(self):
                    ...
                    if not article["article_text"]:
                        ...
                        if article["article_text"]["text"]:
                            ...
                            obj = DbStruct.articles(
                                ...
                                author=article["author"],
                                brief=article["brief"],
                                article=mdformat.text(md=text, extensions=["gfm"]),
                            )
                        else:
                            obj = DbStruct.articles(
                                img_url=article["img_url"],
        ...
        for article in sending_articles:
            ...
            if "onion" in article["img_url"] or str(article["source"]) in [
                "zalaqa_news"
            ]:
                ...
                if d["path"]:
                    ...
                    if d["media_type"] == "video":
                        messages = await self.send(
                            ...
                            message=f"🎥[رابط تحميل الفيديو المرفق]({d['path']})🎥",
                            embed=embed,
                        )
                    elif d["media_type"] == "image":
                        messages = await self.send(
                            channels=send_channels_list, embed=embed, file=d["path"]
                        )
                ...
                if article_data["text"]:
                    ...
                    for message in messages:
                        ...
                        for text in article_text:
                            ...
                            await t.send(
                                content=mdformat.text(md=text, extensions=["gfm"])
                            )
                            ...
                            for vid in article_data["videos"]:
                                ...
                                if d["media_type"] == "video":
                                    ...
                                    if d["media_type"] == "image":
                                        d = discord.File(d["path"]).filename = (
                                            "SPOILER_" + d["path"].filename
                                        )
                                        await t.send(file=d)
            except Exception as e:
                logger.error(e)
