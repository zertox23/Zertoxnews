...
class Scraper:
    ...
    def get_last_news(self):
        ...
        results = soup.find(
            "div", class_="listing listing-grid listing-grid-1 clearfix columns-3"
        )
        ...
        for article in res:
            ...
            images, videos = self.scrape_images_and_videos(
                art, url=self.shahada_agency_url
            )
    ...
    def get_last_amaq_news(self):
        ...
        for article in articles_res:
            ...
            soup = BeautifulSoup(r.content, "html.parser")
            art = soup.find("article")
            images, _ = self.scrape_images_and_videos(
                art, url=self.amaq_url.replace("?cat=467", "")
            )
            ...
            articles.append(
                Article(
                    ...
                    url=url,
                    date=date,
                    author="Amaq News Agency | وكاله اعماق الاخباريه",
                    brief=brief,
                    article_text={"text": text, "images": images, "videos": videos},
                    source="amaq_agency",
                )
            )
    ...
    def get_last_dawla_news(self):
        ...
        for article in articles_res:
            ...
            art = soup.find("article")
            images, videos = self.scrape_images_and_videos(
                art, url=self.dawal_url.replace("?cat=485", "")
            )
            text = markdownify.markdownify(art.prettify())
            articles.append(
                Article(
                    ...
                    url=url,
                    date=date,
                    author="Dawla News | أخبار الدوله",
                    brief=brief,
                    article_text={"text": text, "images": images, "videos": videos},
                    source="dawla_news",
                )
    ...
    def get_last_zalaqa_news(self):
        ...
        for article in articles_res:
            ...
            articles.append(
                Article(
                    title="انفوجرافيك الزلاقه",
                    img_url=img,
                    url=url,
                    date=date,
                    author="Zalaqa | الزلاقه",
                    brief="",
                    article_text={"text": "", "images": [], "videos": []},
                    source="zalaqa_news",
                )
            )
    ...
    def get_last_almirsad_news(self):
        ...
        for article in res:
            ...
            removeddiv = soup.find(
                "div", class_="aioseo-author-bio-compact"
            )  # a div that i don't need in the final text
            removeddiv.extract()
            ...
            articles.append(
                Article(
                    ...
                    url=url,
                    date=date,
                    author="Almirsad Agency | وكاله المرصاد",
                    brief="",
                    article_text={"text": text, "images": [], "videos": []},
                    source="almirsad_news",
                )
            )
